...
func Test_saveMsgInDB(t *testing.T) {
	...
	th.Assert(t, err == nil, err)
	th.Assert(t, validationCount == 8, fmt.Sprintf("Should be 8 validation entries for ID %d, is instead %d", valID1, validationCount))
	...
	err = valResRows.Scan(&validationCount)
	th.Assert(t, err == nil, err)
	th.Assert(t, validationCount == 8, fmt.Sprintf("Should be 8 validation entries for ID %d, is instead %d", valID2, validationCount))

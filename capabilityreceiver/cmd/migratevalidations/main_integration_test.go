...
func Test_addToValidationTableHistory(t *testing.T) {
	...
	th.Assert(t, err == nil, fmt.Sprintf("Err should be nil, is instead %s", err))
	th.Assert(t, valCount == 8, fmt.Sprintf("there should be 8 entries in the validations table with id %d, instead there are %d", valID, valCount))
	...
	err = valRow.Scan(&valCount)
	th.Assert(t, err == nil, fmt.Sprintf("Err should be nil, is instead %s", err))
	th.Assert(t, valCount == 8, fmt.Sprintf("there should only be 8 entries in the validations table with id %d, instead there are %d", valID, valCount))
}

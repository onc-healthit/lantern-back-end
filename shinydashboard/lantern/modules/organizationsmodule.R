...
organizationsmodule <- function(
  ...
  selected_npi_orgs <- reactive({
    ...
    current_confidence <- sel_confidence()
    
    req(current_fhir, current_vendor, current_confidence)
    
    # Get filtered data from the materialized view function
    filtered_data <- get_npi_organization_matches(
      db_connection, 
      fhir_version = current_fhir, 
      vendor = current_vendor, 
      confidence_range = current_confidence
    )
 ...
 selected_endpoint_list_orgs <- reactive({
  ...
  output$npi_orgs_table <- reactable::renderReactable({
    # Get all data
    display_data <- selected_npi_orgs()
    ...
    reactable(
      display_data %>% 
        select(organization_name, url, npi_id, zipcode, organization_secondary_name, fhir_version, vendor_name, match_score) %>% 
        distinct(organization_name, url, npi_id, zipcode, organization_secondary_name, fhir_version, vendor_name, match_score) %>% 
  ...
  output$location_map <- renderLeaflet({
    map_data <- selected_fhir_endpoints()
    
    if (nrow(map_data) == 0) {
      # Return empty map with message if no data
      map <- leaflet() %>%
        addProviderTiles(providers$CartoDB.Positron) %>%
        setView(-98.9, 37.7, zoom = 4) %>%
        addControl(html = "<p style='background-color: white; padding: 10px;'><strong>No data matching the selected filters</strong></p>", 
                   position = "topright")
    ...
    map <- leaflet() %>%
      ...
      addCircles(data = map_data, 
                lat = ~ lat, 
                lng = ~ lng, 
                popup = paste(map_data$organization_name, "<br>", map_data$url),  
                weight = 10, 
                color = "#33bb33", 
                fillOpacity = 0.8, 
                fillColor = "#00ff00") %>%

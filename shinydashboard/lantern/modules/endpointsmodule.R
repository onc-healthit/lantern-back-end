...
endpointsmodule <- function(
  ...
  selected_fhir_endpoints_download <- reactive({
    res <- get_download_fhir_endpoints_tbl(db_tables) %>% select(-http_response, -label)
    req(sel_fhir_version(), sel_vendor())
    if (sel_fhir_version() != ui_special_values$ALL_FHIR_VERSIONS) {
      res <- res %>% filter(fhir_version == sel_fhir_version())
    }
  ...
  output$download_data <- downloadHandler(
    ...
    content = function(file) {
      write.csv(selected_fhir_endpoints_download(), file, row.names = FALSE)
    }
  )
}

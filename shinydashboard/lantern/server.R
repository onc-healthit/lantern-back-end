...
function(input, output, session) { #nolint
  ...
  output$show_filters <- renderUI({
    if (show_filter()) {
      fhirDropdown <- pickerInput(inputId = "fhir_version", label = "FHIR Version:", choices = isolate(app$fhir_version_list()), selected = isolate(app$fhir_version_list()), options = list(`actions-box` = TRUE, `multiple-separator` = " | ", size = 5), multiple = TRUE)

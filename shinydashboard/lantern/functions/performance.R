  ...
  all_endpoints_response_time <- as_tibble(
    tbl(db_connection,
        sql("SELECT floor(extract(epoch from fhir_endpoints_info_history.entered_at)/240)*240 AS time, AVG(fhir_endpoints_info_history.response_time_seconds)
            FROM fhir_endpoints_info, fhir_endpoints_info_history
            WHERE fhir_endpoints_info.id = fhir_endpoints_info_history.id
            AND fhir_endpoints_info_history.entered_at BETWEEN '2020-01-01T00:00:00Z' AND '2020-08-01T00:00:00Z'
            GROUP BY time
            ORDER BY time")
        )
    ) %>%

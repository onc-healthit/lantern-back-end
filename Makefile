...
lint_go:
	...
	cd ./fhir; golangci-lint run -E gofmt --deadline=3m
...
test_e2e_CI:
	docker-compose down
	docker-compose -f docker-compose.yml -f docker-compose.override.yml -f docker-compose.test.yml up --abort-on-container-exit postgres lantern-mq endpoint_manager capability_querier capability_receiver lantern-e2e --exit-code-from lantern-e2e || docker-compose -f docker-compose.yml -f docker-compose.override.yml -f docker-compose.test.yml down && exit 1
	docker-compose -f docker-compose.yml -f docker-compose.override.yml -f docker-compose.test.yml down

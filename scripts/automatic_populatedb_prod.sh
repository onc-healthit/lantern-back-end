...
CHPL_CSV_PATH="/etc/lantern/resources/prod_resources/chpl_service_base_url_list.csv"
...
if [ -f "$CHPL_CSV_PATH" ] && [ -s "$CHPL_CSV_PATH" ]; then
  ...
  docker exec lantern-back-end-endpoint_manager-1 /bin/sh -c "cd /go/src/app/cmd/chplsharedlistsources && go run main.go $CHPL_CSV_PATH" || {

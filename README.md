...
2. **If you have a clean database or want to update the data in your database** 
    1. Run the following command with the Lantern project running to update your endpoint resource files found in `lantern-back-end/resources/prod_resources`. This command will automatically query all the endpoint sources listed in EndpointResourceList.json, which can be found in `lantern-back-end/resources/prod_resources`. It will also query CHPL for it's list of endpoint list sources and NPPES for their endpoint and npi data files and automatically store this NPPES information in the database.
    -Note: The NPPES npidata_pfile and endpoint_pfile are very large and therefore are not persisted in our directory of resources, so you must run this command before running the project for the first time or with a clean database. The query_NPPES-resources script that is a part of this command will query NPPES for their endpoint and npi data files, and it will then cut out all the entries in the npi data file that are not organization entries. It will also create a copy of each file and reduce them to 1000 lines for development resources. The script will then automatically add these NPPES resources to the database and delete the files from the prod and dev resource directories when finished.
     ```bash
...
|`make update_source_data` | Automatically queries the endpoint lists listed in the EndpointResourcesList.json file found in the `resources/prod_resources` directory, including Epic, Cerner, CareEvolution, and 1UpHealth, queries the NPPES npi and endpoint data and automatically adds this information to the database, and queries CHPL for its list of endpoint lists and stores the data in a file in the `resources/prod_resources` directory. |

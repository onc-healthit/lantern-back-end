...
| `make json_export file=<export file name> exportType=<month/30days/all/range> start=<start date> end=<end date>` | Exports the history of the endpoint data to a JSON file specified by the 'file' parameter. This 'file' parameter must only be a file name with the appropriate `.json` file extension, not a file path. Setting exportType equal to "month" creates the export file using only the last months history data, setting it to "30days" or leaving it blank will create the export file with all the history information from the last 30 days, and setting it to "all" will create an export file using all of the history data Lantern has stored. Setting the export type to "range" will create the export file with the history data from the given start date to end date. The start and end parameters are only used for the "range" option, and the end date is non-inclusive (meaning that `end=2021-07-01` would include data up to '2021-06-30/ but not of '07-01'). The start and end date format is '2021-01-31' (year, month, date). Example: `make json_export file=download.json exportType=range start=2021-06-01 end=2021-07-01` |

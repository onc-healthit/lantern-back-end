...
-- Modify the timestamp trigger to respect metadata setting
CREATE OR REPLACE FUNCTION trigger_set_timestamp()
RETURNS TRIGGER AS $$
BEGIN
...
-- Then update the history trigger
...
BEGIN
    ...
    -- For other updates, check if anything besides updated_at changed
    IF (TG_OP = 'UPDATE') THEN
        IF (
            ...
            NEW.metadata_id IS NOT DISTINCT FROM OLD.metadata_id AND
            NEW.requested_fhir_version IS NOT DISTINCT FROM OLD.requested_fhir_version AND
            NEW.capability_fhir_version IS NOT DISTINCT FROM OLD.capability_fhir_version
        ) THEN
            RETURN NEW;
        END IF;
...
DROP TRIGGER IF EXISTS set_timestamp_fhir_endpoints_info ON fhir_endpoints_info;
...
CREATE TRIGGER add_fhir_endpoint_info_history_trigger
    AFTER INSERT OR UPDATE OR DELETE ON fhir_endpoints_info
    FOR EACH ROW
    EXECUTE FUNCTION add_fhir_endpoint_info_history();
...
func Test_ParseNPIdataLine(t *testing.T) {
	...
	reader, file1, err := readCsv(ctx, "testdata/npidata_pfile_fixture.csv")
	defer file1.Close()
	if err != nil {
	...
	line, err = reader.Read()
	line, err = reader.Read()
	th.Assert(t, err == nil, fmt.Sprintf("Expected Read not to throw an error, was %v", err))
...
func Test_ReadCSVContextCancel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	lines, file2, err := readCsv(ctx, "testdata/npidata_pfile_fixture.csv")
	defer file2.Close()
...
func Test_BuildNPIOrgFromNPICsvLine(t *testing.T) {
	...
	// fixture file generated by running `head -20` on the provider organization .csv downloaded from http://download.cms.gov/nppes/NPI_Files.html
	reader, file3, err := readCsv(ctx, "testdata/npidata_pfile_fixture.csv")
	defer file3.Close()
	if err != nil {
		t.Errorf("Error reading NPI data from fixture file")
	}

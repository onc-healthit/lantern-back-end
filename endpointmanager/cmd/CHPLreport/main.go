...
func main() {
	...
	COPY (
		SELECT
   			...
   			string_agg(DISTINCT NULLIF(metadata.http_response,200)::varchar, ',') AS server_status,
		...
		LEFT JOIN (
    		SELECT
    			url, requested_fhir_version, metadata_id, capability_statement::jsonb, updated_at
    		FROM
    		fhir_endpoints_info_history 
		) AS hist ON f.url = hist.url AND f_info.requested_fhir_version = hist.requested_fhir_version
